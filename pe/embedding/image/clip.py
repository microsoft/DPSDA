...
class CLIP(Embedding):
    ...
    def __init__(self, res=None, device="cuda", batch_size=2000):
        """Constructor.
        ...
        self._device = device
        self._clip = CLIP_fx("ViT-B/32", device=device)
        if res is not None:
        ...
        self._resizer = img_preprocess_clip
    ...
    def compute_embedding(self, data):
        ...
        for i in tqdm(range(0, len(x), self._batch_size)):
            ...
            embeddings.append(self._clip(torch.from_numpy(transformed_x).to(self._device)))
